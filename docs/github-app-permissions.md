...
| Secret scanning alerts | Read | Feature evidence where required |
...
`actions/create-github-app-token` step. It passes the short-lived result to
`segh` through `GH_TOKEN`. `segh` never accepts an App private key,
installation ID, or publication credential.
