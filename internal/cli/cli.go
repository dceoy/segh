...
func runAudit(ctx context.Context, args []string, stdout, stderr io.Writer) error {
	...
	if *reconcileSourceScan {
		...
		return runSourceScanReconciliation(*scanManifestPath, *scanResultsDirectory, *scanSummaryPath, *scanMarkdownPath, stdout)
	}
	...
	inventoryCtx, cancelInventory := context.WithTimeout(ctx, time.Duration(cfg.Inventory.Timeout))
	...
	var planErr error
	if cfg.SourceScan.Enabled {
		planCtx, cancelPlan := context.WithTimeout(ctx, time.Duration(cfg.Inventory.Timeout))
