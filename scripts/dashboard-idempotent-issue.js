...
const {list, repositoryId, retryable} = require("./dashboard-github-common.js");
...
async function findIssue(github, owner, repo, id) {
  const matches = (await list(github.rest.issues.listForRepo, {owner, repo, state: "all"}))
    .filter((issue) => !issue.pull_request && repositoryId(issue.body) === id);
...
function idempotentCreate(github) {
  ...
  return async (params) => {
    const id = repositoryId(params.body);
